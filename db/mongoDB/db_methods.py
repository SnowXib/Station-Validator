from .db_consts import OBJECTS_COLLECTION, BROKER_CONF_COLLECTION, TESTS_COLLECTION, TESTS_RUN_COLLECTION, OBJECTS_TEST_COLLECTION
...
def update_tests_run(db, tests_run):
    collection = db[TESTS_RUN_COLLECTION]
    return collection.update_one({"id": tests_run['id']}, {"start_time": tests_run['start_time']})
...
def get_tests_cases(station: list, count: int, db, object_name=None, id=None):
    ...
    if object_name is not None:
        cursor = collection.find({"station": station, "object_name": object_name}, {'_id': 0})
    else:
        cursor = collection.find({"station": station}, {'_id': 0})
