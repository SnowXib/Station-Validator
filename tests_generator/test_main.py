...
def mock_dependencies():
    with (
        patch("tests_generator.main.get_objects") as mock_get_objects,
...
def test_happy_path(mock_dependencies):
    ...
    assert (
        mock_dependencies["simple_gen"].call_count
        == count_of_tests - count_of_script_tests
    )
    ...
    mock_dependencies["send"].assert_has_calls(
        [call(simple_tests_list, db), call(script_tests_list, db)]
    )
...
def test_simple_validation_fails(mock_dependencies):
    ...
    mock_dependencies[
        "simple_gen"
    ].return_value.__next__.side_effect = simple_tests_list
...
def test_script_validation_fails(mock_dependencies):
    ...
    mock_dependencies[
        "script_gen"
    ].return_value.__next__.side_effect = script_tests_list
    mock_dependencies["validate"].side_effect = [
        (True, None),
        (False, "Invalid script test"),
    ]
    mock_dependencies["simple_schema"].return_value = "simple_schema"
    mock_dependencies["script_schema"].return_value = "script_schema"
...
def test_zero_count_of_tests(mock_dependencies):
    ...
    mock_dependencies["script_gen"].assert_not_called()
    mock_dependencies["validate"].assert_has_calls(
        [call([], "simple_schema"), call([], "script_schema")]
    )
    mock_dependencies["send"].assert_has_calls([call([], db), call([], db)])
