...
def parsing_objects(objects_conf_path: str) -> list[dict[str, str]]:
	...
	object_list = [{'object': x['name'], 'type': x['type']} for x in objects_dict['items']]
...
def get_filenames(objects_path: str) -> list[str]:
	...
	:rtype: list[str]
	"""
	print(f"{Fore.BLUE}STEP:{Style.RESET_ALL} Получение списка имен файлов объектов станции")
...
def add_objects_into_type(objects_types: list[dict], object_list: list, station_name: str):
	"""
