...
def setup_broker(broker, port: int, username: str, password: str, db, station: str):
    ...
    if not conf:
        ...
        print(
            f"{Fore.LIGHTGREEN_EX}STEP:{Style.RESET_ALL} Успешное подключение к брокеру: {broker}"
        )
        return client
    ...
    if conf_username and conf_password and conf_broker:
        ...
        print(
            f"{Fore.LIGHTGREEN_EX}STEP:{Style.RESET_ALL} Успешное подключение к брокеру: {conf_broker}"
        )
