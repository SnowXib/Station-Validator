...
from station_test_server.tests_sender import run_test, run_simple_test, run_script_test, get_objects_state
...
def test_run_simple_test_invalid(mock_sleep, mock_mqtt_client, mock_get, mock_put, simple_test_data):
    ...
    mock_client_instance.subscribe.assert_called_once_with("Client/data/CPU/test_object/variables")
...
@patch('station_test_server.tests_sender.mqtt.Client')
@patch('station_test_server.tests_sender.time.sleep')
def test_run_simple_test_state_already_correct(mock_sleep, mock_mqtt_client, mock_get, mock_put, simple_test_data):
    mock_get_response = Mock()
    mock_get_response.status_code = 200
    mock_get_response.json.return_value = {'objectState': 'setup_value'}
    mock_get.return_value = mock_get_response
...
@patch('station_test_server.tests_sender.mqtt.Client')
@patch('station_test_server.tests_sender.time.sleep')
def test_run_simple_test_no_object_name(mock_sleep, mock_mqtt_client, mock_get, mock_put, simple_test_data):
...
@patch('station_test_server.tests_sender.mqtt.Client')
@patch('station_test_server.tests_sender.time.sleep')
def test_run_simple_test_failed_setup(mock_sleep, mock_mqtt_client, mock_get, mock_put, simple_test_data):
    mock_get_response = Mock()
    mock_get_response.status_code = 200
