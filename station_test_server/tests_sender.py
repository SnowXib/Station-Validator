        ...
        if test["result"] not in ["VALID", "NOT_STARTED"]:
            if test["test_type"] == "simple":
                test = run_simple_test(
                    test, test.get("test_interval", 2), test.get("wait_seconds", 2)
                )
...
def run_simple_test(test, test_interval=2, wait_seconds=5):
    ...
    print(
        f"Запускаем тест для CPU/{test['object_name']} с variable {test['variable']} и value {test['value']}"
    )
    ...
    if state != test["value_for_setup"]:
        ...
        if not req.status_code == 200:
            print(
                f"Объект CPU/{test['object_name']} не переведен в состояние {test['value_for_setup']}"
            )
            test["result"] = "ERROR"
            return test
        else:
            print(
                f"Объект CPU/{test['object_name']} переведен в состояние {test['value_for_setup']}"
            )
        payload["variableValue"] = test["value"]
        time.sleep(2)
    elif state == "ERROR":
        print(
            f"Объект CPU/{test['object_name']} возвращает ошибку по переменной {test['variable']}"
        )
        test["result"] = "ERROR"
        return test
