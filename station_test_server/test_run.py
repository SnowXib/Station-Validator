...
def mock_dependencies():
    with (
        patch("station_test_server.run.mongo_connect") as mock_mongo_connect,
...
def test_happy_path(mock_dependencies):
    ...
    mock_dependencies["mongo_connect"].assert_called_once_with(
        "mongodb://test", "test_db"
    )
    mock_dependencies["setup_broker"].assert_called_once_with(
        "test_broker", 1234, "broker_user", "broker_pass", mock_db, "TestStation"
    )
    ...
    mock_dependencies["core"].assert_called_once_with(
        "/path/to/objects", "/path/to/types", "TestStation", mock_db, mock_broker
    )
    mock_dependencies["uvicorn"].run.assert_called_once_with(
