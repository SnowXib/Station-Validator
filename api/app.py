...
from db.mongoDB.db_methods import (get_aggregation_objects, send_tests_cases, get_tests_cases, 
                                   drop_tests_collection, update_tests_results, send_tests_run,
                                   get_tests_run, get_test_by_id, get_tests_run_by_id,
                                   send_object_test, get_objects_test, get_tests_by_ids_db,
                                   save_test_to_db)
...
app = FastAPI(title="Station_test_server", description="Информационная система автоматизированной проверки работоспособности ПАК железнодорожных станций")
...
def generate_tests_case(station, test_run_id="TEST", count=1, db = Depends(get_db)):
    ...
    tests = [next(simple_test_generator(objects, test_run_id)) for _ in range(int(count))]
...
def generate_tests_case(station, test_run_id="TEST", count=1, db = Depends(get_db)):
    ...
    objects = get_aggregation_objects(db, station)
    tests = [next(script_test_generator(objects, test_run_id)) for _ in range(int(count))]
    send_tests_cases(tests, db)
    for test in tests:
        test.pop('_id', None)
...
def get_tests_by_ids(
    id_test: str = Query(description="ID тестов через запятую"),
    db=Depends(get_db)
):
    """
...
def generate_test_run(station, count, db = Depends(get_db)):
    ...
    simple_tests = [next(simple_test_generator(objects, test_run_id)) for _ in range(int(count)-count_script_tests)]
    script_tests = [next(script_test_generator(objects, test_run_id)) for _ in range(count_script_tests)]
    ...
    try:
        ...
        if test_interval_value is not None:
            if isinstance(test_interval_value, str):
                test_interval_value = float(test_interval_value) if '.' in test_interval_value else int(test_interval_value)
        else:
        ...
        if wait_seconds_value is not None:
            if isinstance(wait_seconds_value, str):
                wait_seconds_value = float(wait_seconds_value) if '.' in wait_seconds_value else int(wait_seconds_value)
        else:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка при создании и запуске теста: {str(e)}"
        )
